	...
	progressFn := func(percent int, speed, uploaded, total string) {
		...
		if percent < 100 && percent > 0 {
			...
			if speedBytes > 0 {
				...
				// Use the authoritative total from UploadTotal (set by onPreUpload if available),
				// falling back to rclone-reported total only if onPreUpload hasn't set it.
				etaTotal := progressBox.UploadTotal
				totalBytes := rclone.ParseHumanizedBytes(etaTotal)
